...
$full=Join-Path $OutputDirectory 'TPL-0.1.2.zip'
...
$files=@('TPL.dll','install.ps1','uninstall.ps1','README.md','LICENSE','PLUGIN_API_PERMISSION.md','THIRD_PARTY_NOTICES.md',
    ...
    'TPL\current.txt','TPL\automatic-updates.txt','TPL\versions\0.1.2\TPL.Runtime.dll',
    'TPL\versions\0.1.2\TPL.Update.ps1','TPL\versions\0.1.2\runtime.json')
...
try {
    foreach($name in @('TPL.Runtime.dll','TPL.Update.ps1','runtime.json')) {
        [void][IO.Compression.ZipFileExtensions]::CreateEntryFromFile($zip,(Join-Path $dist ('TPL\versions\0.1.2\'+$name)),$name)
    }
} finally {$zip.Dispose()}
